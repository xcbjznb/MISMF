...
import MISMF as archs
# import archs
import losses
from dataset import Dataset
from metrics import iou_score
from utils import AverageMeter, str2bool
from archs import MISMF
import numpy as np
...
def parse_args():
    ...
    # dataset
    parser.add_argument('--dataset', default='./dataset/Kvasir',
                        help='dataset path')
    parser.add_argument('--img_ext', default='.jpg',
                        help='image file extension')
    parser.add_argument('--img_ext2', default='.png',
                        help='image file extension')
    ...
    parser.add_argument('--optimizer', default='Adam',
                        ...
                        help='loss: ' +
                             ' | '.join(['Adam', 'SGD']) +
                             ' (default: Adam)')
    parser.add_argument('--lr', '--learning_rate', default=1e-3*0.7, type=float,
...
def euclidean_distance(x, y):
    ...
    euclidean_dist = torch.sqrt(sum_sq_diff)  
...
def train(config, train_loader, model, criterion, optimizer):
    ...
    for input, input2, target, _ in train_loader:
        input = input.cuda()
        input2 = input2.cuda()
        target = target.cuda()
        # compute output
        if config['deep_supervision']:
            outputs, list1, list2 = model(input, input2)
            loss = 0
            ...
            output, list1, list2 = model(input, input2)
...
def validate(config, val_loader, model, criterion):
    ...
    with torch.no_grad():
        ...
        for input, input2, target, _ in val_loader:
            input = input.cuda()
            input2 = input2.cuda()
            target = target.cuda()
...
def main():
    ...
    train_path = os.path.join(config['dataset'],'train', 'images', '*' + config['img_ext'])
    train_img_ids = glob(train_path)
    train_img_ids = [os.path.splitext(os.path.basename(p))[0] for p in train_img_ids]

    train_path2 = os.path.join(config['dataset'],'train', 'images2', '*' + config['img_ext2'])
    train_img_ids2 = glob(train_path2)
    train_img_ids2 = [os.path.splitext(os.path.basename(p))[0] for p in train_img_ids2]
    ...
    train_dataset = Dataset(
        ...
        img_ids2=train_img_ids2,
        img_dir=os.path.join(config['dataset'],'train', 'images'),
        img_dir2=os.path.join(config['dataset'], 'train', 'images2'),
